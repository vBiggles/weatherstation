...
void printWeather()
{
  ...
  // Build string with all data from sensors
  String WeatherString = ( \
    "$,winddir=" + String(winddir) + \
    ",windspeedmph=" + String(windspeedmph) + \
    ",windgustmph=" + String(windgustmph) + \
    ",windgustdir=" + String(windgustdir) + \
    ",windspdmph_avg2m=" + String(windspdmph_avg2m) + \
    ",winddir_avg2m=" + String(winddir_avg2m) + \
    ",windgustmph_10m=" + String(windgustmph_10m) + \
    ",windgustdir_10m=" + String(windgustdir_10m) + \
    ",humidity=" + String(humidity) + \
    ",tempc=" + String(tempc) + \
    ",rainin=" + String(rainin) + \
    ",dailyrainin=" + String(dailyrainin) + \
    ",pressure=" + String(pressure) + \
    ",batt_lvl=" + String(batt_lvl) + \
    ",light_lvl=" + String(light_lvl) + \
  ...
  // Format GPS Date String
  sprintf(sz, "%02d/%02d/%02d", gps.date.day(), gps.date.month(), gps.date.year());
  // Add this to the weather string
  WeatherString = WeatherString + ",date=" + String(sz);

  // Format GPS Time String
  sprintf(sz, "%02d:%02d:%02d", gps.time.hour(), gps.time.minute(), gps.time.second());
  // Add this to the weather string
  WeatherString = WeatherString + ",time=" + String(sz);

  // Add tail to Weather String
  WeatherString = WeatherString + ",#";
  
  // Create CharArray for I2C message
  char WeatherChar;
  WeatherString.toCharArray(WeatherChar, WeatherString.length());
  
  // Send Data down I2C for LoRa or other like MQTT etc.
  // We need to use a comms gateway as we've run out of memory
  // on an arduino UNO
  Wire.beginTransmission(1);
  Wire.write(WeatherChar);
  Wire.endTransmission();

  // Print the string out to the Serial Console
  Serial.println();
